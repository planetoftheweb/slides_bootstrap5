...
import Menu from './js/slideList.json'
...
WebFont.load({
  google: {
    families: [
      'Source Code Pro:400',
      'Source Sans Pro:200,300,400,500,700,800,900'
    ]
  }
})
...
if (getUrlVars().d) {
  document.querySelector('section').dataset.markdown =
    'slides/' + dataFile + '.md'
} else {
  dataFile = 'index'
}
...
Reveal.addKeyBinding(
  { keyCode: 84, key: 'T', description: 'Toggle footer display' },
  () => {
...
Reveal.addKeyBinding(
  { keyCode: 77, key: 'M', description: 'Toggle menu display' },
  () => {
...
Reveal.on('ready', () => {
  ...
  mySlides.forEach(currSlide => {
    ...
    let hasFragments = null
      ...
      ; (currSlide.dataset.state &&
        currSlide.dataset.state.includes('no-fragment')) ||
        !Reveal.getConfig().fragments
        ? (hasFragments = false)
        : (hasFragments = true)
